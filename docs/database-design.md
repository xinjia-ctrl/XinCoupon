...
- 第一阶段先保证单体 MVP 闭环，分片能力以路由规则和可选分表脚本形式预置。
...
当前已为高频表预置分片路由和分表脚本，默认仍使用原逻辑表保证项目可直接启动。

| 逻辑表 | 分片键 | 默认分表数 | 说明 |
| --- | --- | --- | --- |
| `user_coupon` | `user_id` | 2 | 用户维度查询和状态流转频繁，适合作为首批分表对象。 |
| `coupon_batch_task` | `batch_no` | 2 | 批量任务天然按批次号隔离，后续可按批次路由。 |

分表脚本见 `docs/sql/008_sharding_tables.sql`，路由说明见 `docs/sharding.md`。
